...
static void nmea_parser_task_entry(void *arg)
{
    ...
    while (1) {
        if (xQueueReceive(esp_gps->event_queue, &event, pdMS_TO_TICKS(200))) {
            ...
            //ESP_LOGI(GPS_TAG, "Evento UART recebido: %d", event.type);
        }
 ...
 */
nmea_parser_handle_t nmea_parser_init(const gps_config_t *config)
{
...
#endif
    esp_gps->all_statements &= 0xFE; //para ignorar “STATEMENT_UNKNOWN”
    /* Set attributes */
    esp_gps->uart_port = config->driver.uart_port;
    esp_gps->event_queue = config->driver.event_queue_hdl;

    /* Set pattern interrupt, used to detect the end of a line */
    uart_enable_pattern_det_baud_intr(esp_gps->uart_port, '\n', 1, 9, 0, 0);
    /* Set pattern queue size */
    uart_pattern_queue_reset(esp_gps->uart_port, config->driver.event_queue_size);
    uart_flush(esp_gps->uart_port);
    /* Create Event loop */
    ...
    BaseType_t err =xTaskCreatePinnedToCore(
        nmea_parser_task_entry,			 // Função da tarefa
        "nmea_parser",			 // Nome da tarefa
        CONFIG_NMEA_PARSER_TASK_STACK_SIZE, // Tamanho do stack
        esp_gps,				 // Parâmetros
        CONFIG_NMEA_PARSER_TASK_PRIORITY,//GPS_TASK_PRIO,		 // Prioridade
        &esp_gps->tsk_hdl,				 // Handle da tarefa
